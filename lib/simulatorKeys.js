...
// Top N overweight (and, optionally, bottom N underweight) sectors for a regime.
export function getSectorTilts(regimeKey, topN = 4) {
  const w = REGIME_SECTOR_WEIGHTS[regimeKey] ?? {};
  const sorted = Object.entries(w).sort((a, b) => b[1] - a[1]);
  return {
    overweight: sorted.slice(0, topN).map(([k, v]) => ({ ...SECTOR_KEYS.find(s => s.key === k), weight: v })),
    underweight: sorted.slice(-topN).reverse().map(([k, v]) => ({ ...SECTOR_KEYS.find(s => s.key === k), weight: v })),
  };
}
