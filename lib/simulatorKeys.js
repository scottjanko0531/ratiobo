...
// GDPNow/ISM Manufacturing New Orders/ISM Services New Orders are new data
// pulls landing alongside this composite rewrite — until fetch-macro-data
// actually ingests them, these three vote null (no data yet), which the
// scoring already handles gracefully via its renormalize-by-available-
// weight step, same as any indicator not yet loaded.
export const NEARTERM_GROWTH_SIGNALS = [
  { label: "GDPNow", name: "GDPNow", w: 0.20, isNowcast: true, vote: v => v > 2.5 ? 1 : v >= 1.0 ? 0 : -1 },
  { label: "ISM Mfg New Orders", name: "ISM Manufacturing New Orders", w: 0.15, vote: v => v > 55 ? 1 : v >= 50 ? 0 : -1 },
  { label: "ISM Services New Orders", name: "ISM Services New Orders", w: 0.10, vote: v => v > 55 ? 1 : v >= 50 ? 0 : -1 },
...
export const NEARTERM_INFL_SIGNALS = [
  ...
  { label: "Tariff Impact", name: "Tariff Inflation Impact", w: 0.05, vote: v => v > 0.30 ? 1 : v > 0.10 ? 0 : -1 },
];
...
export const MEDTERM_INFL_SIGNALS = [
  { label: "10Y Breakeven", name: "10Y Breakeven Inflation", w: 0.26, vote: v => v > FED_INFLATION_TARGET ? 1 : v >= 1.5 ? 0 : -1 },
...
// Tiebreaker for the Structural regime's growth/inflation axis when Rate of
// Change reads "Stable" (the fast/slow gap didn't clear minGap in either
// direction). The four-quadrant model needs a binary Up/Down, so Stable
// can't just silently collapse into Down the way isGrowthAccelerating/
// isInflationAccelerating do on their own — that's the same "Stable isn't
// Down" bug the regime tiebreaker spec reports, just one level up from the
// dead-band fix. Falls back, in order:
//   1. Rate of Change itself (Accelerating/Decelerating) — no tiebreak needed.
//   2. Direction (G3/I3 — the fast indicator's own short-window sign).
//   3. Level (G1/I1 — z-score vs the long window): above the window mean
//      leans Up, below leans Down.
// `tentative` is true whenever the axis had to fall through to Direction or
// Level — the UI uses this to mark a tiebreak-resolved regime read
// differently from a clean, direct crossover match, per the spec's "don't
// present a coin-flip-resolved label with full confidence" requirement.
export function resolveAxisDirection(fast, slow, minGap, direction, zscore) {
  const rateLabel = rateOfChangeLabel(fast, slow, minGap);
  if (rateLabel === "Accelerating") return { up: true, tentative: false, resolvedVia: "rate", rateLabel };
  if (rateLabel === "Decelerating") return { up: false, tentative: false, resolvedVia: "rate", rateLabel };
  if (direction === 1) return { up: true, tentative: true, resolvedVia: "direction", rateLabel };
  if (direction === -1) return { up: false, tentative: true, resolvedVia: "direction", rateLabel };
  if (zscore != null && zscore !== 0) return { up: zscore > 0, tentative: true, resolvedVia: "level", rateLabel };
  return { up: null, tentative: true, resolvedVia: null, rateLabel };
}
