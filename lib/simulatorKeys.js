...
//       newVal, deltaVal, isIlliquid, key, exposureMultiplier }, sorted by
//       |deltaVal| descending. A bucket's target % is distributed across its
//       holdings proportionally to each holding's current share of that
//       bucket, THEN scaled by that holding's own exposureMultiplier (the
//       VAMS-equivalent Bottom-Up resize overlay — see asset_resize_signals/
//       asset_resize_rule_config) — a Top-Down regime target cut further by a
//       symbol-specific risk-state signal, same two-stage design as KISS's
//       Target Allocation x VAMS multiplier. Illiquid buckets never get a
//       negative newVal (never recommend selling them down) — that floor
//       applies to the un-multiplied target, so a resize multiplier can't be
//       read as "sell an illiquid holding."
//   - sectorTargets (optional): { symbol: targetPctOfBucket } — overrides the
//     default within-bucket split (pro-rata by current value) for specific
//     symbols, e.g. a regime-driven equity-sector tilt where XLE should get
//     4% of the eq bucket's target regardless of how much XLE is currently
//     held. Falls back to pro-rata for any symbol not in the map, so mixing
//     sector-tilted and untilted holdings in the same bucket is safe.
//   - buyRows: buckets with a nonzero target and no current holdings.
export function computeAllocationDeltas(holdings, suggestedPcts, { illiquidKeys = new Set(), exposureMultipliers = {}, sectorTargets = {}, includeZeroValueHoldings = false } = {}) {
  const byKey = {};
  let grandTotal = 0;
  for (const h of holdings ?? []) {
    const val = Number(h.current_value ?? 0);
  ...
  // Every key with a holding actually linked to this call's `holdings` array,
  // positive value or not. Only consulted when includeZeroValueHoldings is
  // set (portfolio-scoped callers) — a symbol resized to 0% by its own live
  // resize signal (e.g. GLDM fully "Reduced") still has a linked holding at
  // $0, and should show up specifically in actionRows (with its Reduced
  // badge and a $0/0% Hold row) rather than vanish, while also not
  // double-triggering the generic bucket-level buyRow below for the same gap.
  const linkedKeys = new Set((holdings ?? []).map((h) => resolveSimulatorKey(h)).filter(Boolean));
  ...
  const actionRows = hasPortfolio && hasTargets
    ? (holdings ?? []).flatMap((h) => {
        ...
        if (currentVal <= 0 && !includeZeroValueHoldings) return [];
        const currentPct = (currentVal / grandTotal) * 100;
        const bucketTargetPct = suggestedPcts[key] ?? 0;
        const bucketTotal = byKey[key]?.total ?? 0;
        const sectorTargetPct = sectorTargets[h.symbol];
        const holdingShare = sectorTargetPct != null
          ? sectorTargetPct / 100
          : (bucketTotal > 0 ? currentVal / bucketTotal : 1);
        const exposureMultiplier = exposureMultipliers[h.symbol] ?? 1;
        const holdingTargetPct = bucketTargetPct * holdingShare * exposureMultiplier;
  ...
  const buyRows = hasPortfolio && hasTargets
    ? Object.entries(suggestedPcts)
        .filter(([k, p]) => p > 0 && !(includeZeroValueHoldings ? linkedKeys.has(k) : Boolean(byKey[k])))
        .map(([k, p]) => ({
          key: k,
          label: KEY_LABEL[k] ?? k,
          targetPct: p,
