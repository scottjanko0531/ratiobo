...
export default function WatchListsPage() {
  ...
  async function runAnalysis(req) {
    ...
    try {
      // Item-scope analysis runs as a multi-stage pipeline (sector -> competitors -> company -> synthesis).
      // Each request performs at most one generation step and reports back whether more remain; a fully
      // cached run resolves in a single round trip. Group-scope stays a single request.
      let forceNext = req.force ?? false;
      let done = false;
      let guard = 0;
      while (!done && guard < 8) {
        guard++;
        ...
        forceNext = false; // only force the first request in the chain; cached stages regenerated by it read as fresh afterward
        if (req.scope === "group" || data.done) {
          setAnalysis({ title: req.title, content: data.content, generated_at: data.generated_at });
          done = true;
        } else if (data.label) {
          setAnalysisProgress(data.label);
        }
      }
      if (!done && guard >= 8) setAnalysisError("Analysis is taking longer than expected — try again.");
    } catch (_) {
