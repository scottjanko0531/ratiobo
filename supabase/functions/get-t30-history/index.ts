...
Deno.serve(async (req: Request) => {
  ...
  try {
    const daily = await fetchYahooDaily("^TYX");
    ...
    // Also expose the most recent daily close as a current-day point
    const latestDaily = daily[daily.length - 1];

    type Row = {
      date: string;
      value: number;
      yoy: number | null;  // pp change vs same month last year
      mom: number | null;  // pp change vs prior month
    };

    const rows: Row[] = [];
    for (let i = 0; i < months.length; i++) {
      const m = months[i];
      const value = Math.round(monthAvg.get(m)! * 1000) / 1000;

      const [yr, mo] = m.split("-").map(Number);
      const yaKey = `${yr - 1}-${String(mo).padStart(2, "0")}`;
      const yaVal = monthAvg.get(yaKey);
      // YoY as percentage point difference (more natural for rates than % change)
      const yoy = yaVal != null ? Math.round((value - yaVal) * 1000) / 1000 : null;

      const prevVal = i > 0 ? monthAvg.get(months[i - 1])! : null;
      const mom = prevVal != null ? Math.round((value - prevVal) * 1000) / 1000 : null;

      rows.push({ date: m + "-01", value, yoy, mom });
    }

    return new Response(JSON.stringify({ rows, latestDaily }), {
